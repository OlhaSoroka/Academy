<template>
  <BaseModal
    ref="homeworkResaltsUpdateModal"
    :header="'Update homework resalts'"
  >
    <template 
      #body 
    >
...
export default {
  ...
  watch: {
    toggleModal() {
      this.$refs.homeworkResaltsUpdateModal.openModal();
    },
    studentId() {
...
this.getCourses().then(() => {
  ...
  methods: {
    ...
    async submit() {
      ...
      this.$refs.homeworkResaltsUpdateModal.closeModal();
    },
    cancel() {
      this.$refs.homeworkResaltsUpdateModal.closeModal();
    },
