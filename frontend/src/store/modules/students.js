...
import {
  createGroupMember,
  updateGroupMember,
} from "@/helpers/createUpdateGroupMember";
import {
  createResultMember,
  updateResultMember,
} from "@/helpers/createUpdateResultMember";
import {
  createHomeworkResaltsMember,
  updateHomeworkResaltsMember,
} from "@/helpers/createNewHomeworkResultsMember";
  ...
  actions: {
    ...
    updateStudent: async (store, data) => {
      try {
        ...
        const studentsCourse = allCourses.find(
          (course) => course.name === student.course
        );
        if (studentsCourse) {
          const groupMember = studentsCourse.group.find(
            (member) => member.email === student.email
          );
          const resultMember = studentsCourse.results.find(
            (member) => member.email === student.email
          );
          const resultHomeworkMember = studentsCourse.homework_results.find(
          ...
          const indexToUpdateGroupMember = studentsCourse.group.findIndex(
            (member) => member.email === student.email
          );
          const indexToUpdateResultMember = studentsCourse.results.findIndex(
            (member) => member.email === student.email
          );
          const indexToUpdateHomeworkResultMember =
            studentsCourse.homework_results.findIndex(
              (member) => member.id === student.id
            );
          studentsCourse.group.splice(
            indexToUpdateGroupMember,
            1,
            updatedGroupMember
          );
          studentsCourse.results.splice(
            indexToUpdateResultMember,
            1,
            updatedResultMember
          );
          studentsCourse.homework_results.splice(
            indexToUpdateHomeworkResultMember,
            1,
            updatedHomeworkResultMember
          );
      ...
      try {
        ...
        if (studentsCourse) {
          ...
          if (studentsCourse.homework_results[0]) {
            homework = JSON.parse(
              JSON.stringify(studentsCourse.homework_results[0].homework)
            );
          ...
          const resultHomeworkMember = createHomeworkResaltsMember(
            student,
            homework
          );
    ...
    deleteStudent: async (store, id) => {
      try {
        store.commit("TOGGLE_LOADING_STATUS");
        const student = store.state.students.find(
        ...
        if (studentsCourse) {
          ...
          const indexToUpdateResultHomeworkMember =
            studentsCourse.homework_results.findIndex(
              (member) => member.id === student.id
            );
          ...
          studentsCourse.homework_results.splice(
            indexToUpdateResultHomeworkMember,
            1
          );
          await updateCourseById(studentsCourse.id, studentsCourse);
        }
