...
import { Comment } from "../models/comment.model";
  ...
  export const updateCommentById = async (id: string, comment: Comment): Promise<boolean> => {
  ...
  export const getCommentsByCourse = async (courseId: string): Promise<Comment[]> => {
	const collectionQuery = query(collection(firestore, Collection.COMMENTS), where("courseId", "==", courseId));
  ...
  export const getCommentsByAuthor = async (authorId: string): Promise<Comment[]> => {
	const collectionQuery = query(collection(firestore, Collection.COMMENTS), where("authorId ", "==", authorId));
	const documents = await getDocs(collectionQuery);
	const comments: Comment[] = [];
	documents.forEach((document) => {
	  const comment = document.data();
